...
def plot(side: int, circ: list[Circ], saveto: str | None = None) -> None:
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate circle regions hypergraph instance."
    )
    parser.add_argument("ncircles", type=int, help="Number of circles.")
    parser.add_argument("radius", type=int, help="Radius as percentage of side.")
    parser.add_argument("ninstances", type=int, help="Number of instances to generate.")
    parser.add_argument("out", type=str, help="Output directory path.")
    parser.add_argument(
        "--side", type=int, default=100, help="Length of the side of the scene."
    )
    parser.add_argument("--nsteps", type=int, default=1000, help="Size of the mesh.")
    parser.add_argument(
        "--integer",
        action="store_true",
        help="Generate integer coordinates for centers.",
    )
    ...
    # Read arguments
    side = args.side
    ncenters = args.ncircles
    radius = args.radius
    ninstances = args.ninstances
    out = args.out
    nsteps = args.nsteps
    real = not args.integer

    # Generate instances
    for i in range(ninstances):
        centers = gen_centers(side, ncenters, real=real, seed=i)
        ...
        name = f"circ_{ncenters}_{radius}_{i}"
        write_hypergraph(
            out + "/" + name + ".cfc",
            (num_vertices, num_edges, edges),
        )
        plot(side, circles, saveto=out + "/" + name + ".png")
