...
static int remove_zero_weigthed(soldata* sol)
{
   ...
   for (i = 0; i < nnodes; ++i) {
      ...
      if (sol->nweights[v] == 0.0) {
         ...
         changes++;
         /* The following line was commented out by M. Lucci (2025) */
         //printf("remove_zero_weigthed\n");
...
static int perform_1_2_path(soldata* sol, int* nodestack, int v)
{
   ...
   /* The following lines were added by M. Lucci (2025) */
   if (sol->ncount < 10)
      return changes;
...
int COLORcheck_set(COLORset* set, int ncount, int ecount, const int elist[])
{
   ...
   for (i = 0; i < ecount; ++i) {
      if (coloring[elist[2*i]] == 1 && coloring[elist[2*i+1]] ==1) {
         fprintf(stderr, "Edge (%d,%d) connects two nodes in stable set!\n",
